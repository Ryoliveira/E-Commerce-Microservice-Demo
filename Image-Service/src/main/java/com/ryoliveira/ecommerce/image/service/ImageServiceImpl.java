...
public class ImageServiceImpl implements ImageService {
	...
	public Image saveImage(MultipartFile file, int productId) {
		Image img = null;
		try {
			img = new Image(productId, file.getOriginalFilename(), file.getContentType(), Base64.getEncoder().encodeToString(file.getBytes()));
			imageRepo.save(img);
	...
	public Image getImage(int productId) {
		...
		Image img = new Image();
		try {
			img = optional.orElseThrow(() -> new NoSuchElementException("No Image with product id:" + productId + " was found"));
	...
	@Override
	public Image updateImage(MultipartFile file, int productId) {
		Optional<Image> optional = imageRepo.findById(productId);
		Image img = new Image();
		try {
			img = optional.orElseThrow(() -> new NoSuchElementException("No Image with product id:" + productId + " was found"));
